...
## Task Types and Execution Directives (Reference for Planning)
...
*   **`Task Type` Definition**: A critical label (e.g., `new-feat`, `bug-fix`, `ref-struct`, `ref-func`) assigned by the Planner to guide the Executor's specific approach to a task.
*   **Mandatory Labeling**: Every task in a task file (e.g., `.cursor/feature-x-tasks.md`) **must** include a `Task Type` label.

**Task Types:**
*   **`ref-struct` (Structural Refactoring):** For tasks requiring structural changes while preserving existing logic and behavior *exactly*. No functional changes or unrelated optimizations.
*   **`new-feat` (New Feature):** For implementing new functionalities.
*   **`bug-fix` (Bug Fix):** For correcting errors in existing code.
*   **`ref-func` (Functional Refactoring):** For tasks that involve changing or improving existing logic or functionality, potentially with code quality, scalability, and maintainability enhancements. This includes splitting long files/functions.
